...
class SqfliteInstance {
  ...
  void createTable() async {
    await _database.execute("CREATE TABLE person(id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, age INTEGER)");
  }
  ...
  void insertData() async{
    ...
    //下面这种方式更加优雅，更方便
    int result = await _database.insert("person", {
      "name": "是Jimi啊",
      "age": 28
    });
    print("插入数据：result的值为$result");
  ...
  void queryData() async{
    ...
    // print(list.toString());
    //第二种：query()
    var list = await _database.query("person",);
    print(list.toString());
  }
  ...
  void updateData() async{
    ...
    //下面这种写法更优雅
    int result = await _database.update("person", {
      "name": "张三"
    }, where: "id = 4");
  }
